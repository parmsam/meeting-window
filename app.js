...
// Maps US state names and abbreviations → 2-letter state code
const STATE_ALIASES = {
...
function searchCities(q) {
  ...
  // Check if query resolves to a US state (takes priority over country codes)
  const resolvedState = STATE_ALIASES[lq];
  if (resolvedState) {
    return CITIES.filter(c => c.state === resolvedState)
